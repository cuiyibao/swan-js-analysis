...
export default class Master {
  constructor(context, swaninterface, swanComponents) {
    ...
    this.pageLifeCycleEventEmitter =
      allSlaveEventEmitters.pageLifeCycleEventEmitter;
  ...
  listenFirstRender() {
    this.swaninterface.invoke("onMessage", data => {
      if (
        data.type === "slavePageComponentAttached" &&
        !firstRenderHookQueue.firstRenderEnd
      ) {
  ...
  preLoadSubPackage() {
    ...
    swan.getNetworkType({
      success: function(res) {
        ...
        //遍历配置项
        Object.keys(appConfig.preloadRule).forEach(key => {
  ...
  decorateContext(context) {
    ...
    context.swan = this.decorateSwan(
      Object.assign(this.swaninterface.swan, context.swan || {})
    );
    context.getCurrentPages = getCurrentPages;
    context.global = {};
    context.Page = Page;

    context.Component = this.virtualComponentFactory.defineVirtualComponent.bind(
      this.virtualComponentFactory
    );

    context.Behavior = this.virtualComponentFactory.defineBehavior.bind(
      this.virtualComponentFactory
    );
  ...
  setGLobalAppInfo() {
    ...
    const appInfo = this.swaninterface.boxjs.data.get({
  ...
  initRender(initEvent) {
    if (
      ...
      this.dispatchJSPromise.then(() =>
        this.navigator.pushInitSlaveToHistory()
      );
  ...
  decorateSwan(originSwan) {
    return apiProccess(originSwan, {
      ...
      hostShareParamsProccess: this.extension.hostShareParamsProccess.bind(
        this.extension
      ),
