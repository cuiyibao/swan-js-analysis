...
export const apiProccess = (
  ...
  const operators = swanComponents.getContextOperators(
    swaninterface,
    communicator,
    getSlaveId
  );
  ...
  return Object.assign(originSwan, {
     ...
     * @return {*} 发送日志后的返回值
     */
    reportAnalytics: (reportName, reportParams) =>
      swanEventsCommunicator.fireMessage({
        type: "SwanEvents",
        params: {
    ...
    openShare: (originShare => userParams => {
      const appInfo = swaninterface.boxjs.data.get({
