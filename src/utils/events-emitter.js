...
class QueueSet {
  ...
  has(namespace) {
    return (
      Object.prototype.toString.call(this.queueSet[namespace]) ===
      "[object Array]"
    );
  }
  ...
  del(namespace, element) {
    if (!element) {
      ...
      Object.keys(this.queueSet)
        ...
        .forEach(queueName => {
          this.queueSet[queueName] = this.queueSet[queueName].filter(
            item => item.handler !== element
          );
        });
    } else {
      this.queueSet[namespace] = this.queueSet[namespace].filter(
        item => item.handler !== element
      );
    }
  }
...
export default class EventsEmitter {
  ...
  fireMessage(message, options = {}) {
    if (message && message.type && this.handlerQueueSet.get(message.type)) {
      ...
      this.handlerQueueSet.get(message.type).forEach(item => {
        this.handlerWrapper(item, message.type, message);
      });

      this.handlerQueueSet.get("*").forEach(item => {
  ...
  onMessage(type, handler, options = {}) {
    ...
    if (
      options.listenPreviousEvent === true &&
      this.messageQueueSet.has(type)
    ) {
      this.handlerWrapper(
