...
export const getABSwitchValue = name => {
  ...
  // webview上绑定之后的逻辑
  else if (window._envVariables && window._envVariables.abTestSwitch) {
...
export const convertToCamelCase = str =>
  str.replace(/-([a-z])/g, (all, first) => first.toUpperCase());
...
export const isEqual = (one, other, oneStack, otherStack) => {
  ...
  if (areArrays) {
    ...
    while (length--) {
      if (!isEqual(one[length], other[length], oneStack, otherStack))
        return false;
    }
  } else {
    ...
    while (length--) {
      ...
      if (
        !(
          ...
          isEqual(one[key], other[key], oneStack, otherStack)
        )
      )
        return false;
    }
...
export const deepAssign = (targetObject = {}, originObject) => {
  ...
  if (originType === "[object Array]") {
    ...
    originType === "[object Object]" &&
    originObject.constructor === Object
  ) {
...
export const deepClone = originObj => {
  return deepAssign(
    Object.prototype.toString.call(originObj) === "[object Array]" ? [] : {},
    originObj
  );
};
