...
export default class SanFactory {
  constructor(componentDefaultProps, behaviors) {
    ...
    const communicator = Communicator.getInstance(
      this.componentDefaultProps.swaninterface
    );
  ...
  createComponents(componentName) {
    ...
    // 获取超类名称
    const superComponentName =
      originComponentPrototype.superComponent || "swan-component";
  ...
  decorateComponentPrototype(componentPrototype, componentInfo) {
    // 所有的组件的依赖在依赖池寻找后的结果
    const dependencies = (componentPrototype.dependencies || []).reduce(
      (dependencies, depsName) => {
  ...
  mergeComponentProtos = (targetProto, mergeProto) => {
    // merge传入的proto
    return Object.keys(mergeProto).reduce(
        ...
        switch (propName) {
          ...
          case "created":
            mergedClassProto[propName] = function(options) {
              targetProto[propName] &&
                targetProto[propName].call(this, options);
          ...
          case "dependencies":
            mergedClassProto[propName] = (mergeProto[propName] || []).reduce(
              (r, v, k) => {
  ...
  defineSanComponent(proto) {
    ...
    Object.keys(proto).forEach(propName => {
  ...
  getComponentsForMap(componentUsingComponentMap) {
    ...
    return Promise.all([
      ...
      const getComponentEvent = function(componentUsingComponentMap) {
        ...
        Object.keys(componentUsingComponentMap).forEach(customAbsolutePath => {
          ...
          if (!customComponentsSizeInfo[customAbsolutePath]) {
            customComponentsSizeInfo[customAbsolutePath] =
              customComponentObj["size"] || 0;
          }
          // 深度遍历加载创建-同步构建
          if (
            Object.keys(customComponentObj.componentUsingComponentMap).length
          ) {
            if (!comstomUsingCompoentMap[customAbsolutePath]) {
              comstomUsingCompoentMap[customAbsolutePath] = {};
              let components = getComponentEvent(
                customComponentObj.componentUsingComponentMap
              );
              Object.assign(
                comstomUsingCompoentMap[customAbsolutePath],
                components
          ...
          customNamesArr.forEach(prefix => {
            ...
            that.componentDefine(
              componentUniqueName,
              Object.assign({}, that.getProtos("super-custom-component"), {
                // superComponent: 'super-custom-component',
                template:
                  "<" +
                  swanPrefix +
                  ">" +
                  customComponentObj.template +
                  "</" +
                  swanPrefix +
                  ">",
                ...
                customComponentCss:
                  that.translateAllCustomImportCss(
                    customComponentObj.customComponentCss,
                    prefix
                  ) || ""
              }),
              {
      ...
      let allCustomComponentsSize = Object.keys(
        customComponentsSizeInfo
      ).reduce((total, customAbsolutePath) => {
...
export const getComponentFactory = (
  ...
  const sanFactory = new SanFactory(
    componentDefaultProps,
    behaviors,
    componentProtos
  );
  ...
  Object.keys(componentProtos).forEach(protoName =>
    sanFactory.componentDefine(protoName, componentProtos[protoName])
  );
