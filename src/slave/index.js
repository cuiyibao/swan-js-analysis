...
export default class Slave {
  ...
  loadUserResource(appPath, pagePath) {
    ...
    this.userResourcePromise = Promise.all([
      ...
      .then(() => {
        ...
        loader
          .loadjs(`${appPath}/${pagePath}.swan.js`, "slaveActiveSwanJsLoaded")
          .then(() => {
            try {
  ...
  listenDispatchJSFiles(global) {
    this.swaninterface.bind("dispatchJSSlave", event => {
      ...
      let { appPath, pagePath } = storeInfoToGlobal(
        event,
        this.context,
        global
      );
      ...
      event.devhook === "true"
        ? loadHook()
            ...
            .catch(this.loadUserResource(appPath, pagePath))
        : this.loadUserResource(appPath, pagePath);
    });
  }
  ...
  andrSendFMP() {
    ...
    if (this.swaninterface.boxjs.platform.isAndroid()) {
      ...
      global.performance.timing.domFirstScreenPaint
        ? sendFMP()
        : setTimeout(sendFMP, 3000);
    }
  }
  ...
  createAllComponents(pageContentInfo) {
    ...
    if (isComponent) {
      ...
      componentUsingComponentMap[pageContentInfo.componentPath] = [
        "custom-component"
      ];
    }
    ...
    let pageComponentUsingCustomComponent = !!Object.keys(
      componentUsingComponentMap
    ).length;
    Promise.resolve()
      .then(() => {
        if (!pageComponentUsingCustomComponent) {
          return {
            customComponents: {},
            pageTemplateComponents: pageContentInfo.createTemplateComponent(
              componentFragments
            )
          };
        } else {
          ...
          getComponentsForMap = componentFactory.getComponentsForMap(
            componentUsingComponentMap
          );
          return getComponentsForMap.then(customComponentsInfo => {
            ...
            let customComponentsSizeInfo =
              customComponentsInfo.customComponentsSizeInfo;
            // 给页面template使用的组件赋值
            Object.assign(
              templateComponents,
              componentFragments,
            ...
            let pageTemplateComponents = pageContentInfo.createTemplateComponent(
              templateComponents
            );
            return {
      ...
      .then(function(pageInfo) {
        window.pageRender(
          pageContent,
          pageInfo.pageTemplateComponents,
          pageInfo.customComponents,
          initialFilters,
          modules,
          "newTemplate",
          pageInfo.customComponentsSizeInfo
        );
      })
      .catch(function(e) {
        window.errorMsg["execError"] = e;
  ...
  registerComponents() {
    ...
    componentFactory.dependenciesPool.communicator.onMessage(
      "initData",
      params => {
        ...
        params => {
          ...
          try {
            ...
            const attachPage = () => {
              ...
              this.andrSendFMP();
              // 通知master加载首屏之后的逻辑
              page.communicator.sendMessage("master", {
                type: "slaveAttached",
    ...
    const compatiblePatch = () => {
      global.PageComponent = global.componentFactory.getComponents(
        "super-page"
      );
      global.PageComponent.components = global.componentFactory.getComponents();
      global.PageComponent.stabilityLog =
        global.PageComponent.stabilityLog || new Function();
    };
    compatiblePatch();
