...
export default class Extension {
  ...
  injectHostMethods(extension) {
    let { hostMethodDescriptions = [], name: hostName } = extension;
    try {
      this.swaninterface.boxjs.extend(
        hostMethodDescriptions.map(description => {
          ...
          let { name = description, path, authority = "swanAPI" } = description;
  ...
  injectHostAPI(extension) {
    ...
    extension.methods &&
      Object.keys(extension.methods).forEach(apiName => {
  ...
  injectHostComponents(extension) {
    ...
    Object.keys(extension.components).forEach(ComponentName => {
      ...
      this.context.componentFactory.componentDefine(
        finalComponentName,
        extension.components[ComponentName]
      );
    });
  ...
  use(context, isMaster) {
    ...
    // 加载js
    loader
      .loadjs(`${extensionPath}/extension.js`)
      .then(() => {
        // 模块引入
